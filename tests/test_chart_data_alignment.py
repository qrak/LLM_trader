...
async def main():
    ...
    try:
        ...
        if last_candle_aligned:
            print("  [PASS] Last candle alignment")
            ...
            print("  [FAIL] Last candle alignment")
            print(f"     Analysis last: timestamp={int(analysis_last[0])}, close={analysis_last[4]:.2f}")
            print(f"     Chart last:    timestamp={int(chart_last[0])}, close={chart_last[4]:.2f}")
            return 1
        ...
        for i, candle in enumerate(closed_candles):
            if np.array_equal(candle, chart_first):
                ...
                print("  [PASS] Chart subset verification")
                print(f"     Chart first candle found at analysis index {i}")
                break
        
        if not chart_first_found:
            print("  [FAIL] Chart subset verification")
            print("     Chart first candle not found in analysis data")
            return 1
        ...
        if all_consistent:
            print("  [PASS] Timestamp continuity: consistent 4h intervals")
        else:
            print("  [WARN] Timestamp continuity: some intervals vary")
