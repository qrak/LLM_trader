...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("0.5%", 0.005),
        (0.05, 0.05),
        (50, None),  # ambiguous without % suffix — rejected
    ],
)
...
def test_long_signal_accepted() -> None:
    ...
    extractor = PositionExtractor(logger=MagicMock())
    assert extractor.validate_signal("LONG") is True


def test_short_signal_accepted() -> None:
    """Futures SHORT signal is recognized as valid."""
    extractor = PositionExtractor(logger=MagicMock())
    assert extractor.validate_signal("SHORT") is True


def test_long_signal_extracted_from_text() -> None:
    """Futures LONG signal parsed from text response."""
    extractor = PositionExtractor(logger=MagicMock())
    response = "signal: LONG confidence: HIGH stop_loss: 95 take_profit: 110 position_size: 5%"
    signal, _, _, _, position_size, _ = extractor.extract_trading_info(response)
    assert signal == "LONG"
    assert position_size == pytest.approx(0.05)
