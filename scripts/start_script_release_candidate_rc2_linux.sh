...
#
# start_script_release_candidate_rc2_linux.sh
# Purpose: Prepare .venv, ensure on 'release/v1.0-rc2' git branch when available, install requirements, and run start.py
# Usage: ./scripts/start_script_release_candidate_rc2_linux.sh [symbol] [-t timeframe] [--skip-install]
#
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --skip-install)
            ...
            ;;
        -h|--help)
            echo "Usage: ./scripts/start_script_release_candidate_rc2_linux.sh [symbol] [-t timeframe] [--skip-install]"
            exit 0
            ;;
        *)
            if [[ -z "${SYMBOL}" ]]; then
...
echo "== scripts/start_script_release_candidate_rc2_linux.sh (${RELEASE_BRANCH}) =="
