...
#
# start_script_main_linux.sh
# Purpose: Prepare .venv, install requirements, and run start.py (main branch)
# Usage: ./scripts/start_script_main_linux.sh [symbol] [-t timeframe] [--skip-install]
#
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --skip-install)
            ...
            ;;
        -h|--help)
            echo "Usage: ./scripts/start_script_main_linux.sh [symbol] [-t timeframe] [--skip-install]"
            exit 0
            ;;
        *)
            if [[ -z "${SYMBOL}" ]]; then
