...
class CryptoCompareDataProcessor:
    ...
    def normalize_categories_data(self, api_categories: Any) -> Optional[List]:
        ...
        if isinstance(api_categories, list):
            self.logger.debug("Processing list with %s items", len(api_categories))
            return api_categories

        self.logger.warning("Unexpected data type for api_categories: %s", type(api_categories))
        return None

    def _extract_data_from_dict(self, data_dict: Dict) -> Optional[List]:
        """Extract category data from dictionary structures"""
        self.logger.debug("Processing dictionary with keys: %s", list(data_dict.keys()))
        ...
        if "Response" in data_dict and "Data" in data_dict:
            if data_dict["Response"] in ("Success", "success"):
                self.logger.debug("Using data from 'Data' key: %s", type(data_dict['Data']))
                return data_dict["Data"]
            else:
                self.logger.warning("API response not successful: %s", data_dict.get('Message', 'Unknown error'))
                return None
