<#
start_script_new_features.ps1
Purpose: Prepare .venv, ensure on 'new_features' git branch (if repo), install requirements, and run start.py
Usage: pwsh.exe -File .\start_script_new_features.ps1 [symbol] [-Timeframe <tf>]
Examples:   
  .\start_script_new_features.ps1                           # Run with default from config.ini
Parameters:
...
Write-Host "== start_script_new_features.ps1 (test) =="

# Checkout branch if this is a git repo
if (Test-Path .git) {
    Write-Host "Switching to 'new_features' branch..."
    git fetch --all --prune
